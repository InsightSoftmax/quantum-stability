...
const data = await FileAttachment("data/ibm-pittsburgh.json").json();
```

# IBM Pittsburgh

IBM superconducting QPU (Heron r3, 156 qubits) accessed via Qiskit Runtime, pay-as-you-go instance (us-east). Weekly runs on Mondays.
...
```js
volatilityTimeSeries(data, {color: "#0043CE"})
```
...
```js
successTimeSeries(data, {color: "#0043CE"})
```
...
```js
successSurface3D(data, {color: "#0043CE"})
```

### Distribution by circuit depth

```js
boxByLength(data, {color: "#0043CE"})
```
...
```js
successByLength(data, {color: "#0043CE"})
```
...
```js
successByInput(data, {color: "#0043CE"})
```
